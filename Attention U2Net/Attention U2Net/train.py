...
wandb.init(project='AttentionU2net_normal',entity='avarude')
...
if __name__ == '__main__':
    ...
    model_name = 'attu2net_normal'  
    

    print(model_name)
    data_dir = os.path.join(os.getcwd(), 'data' + os.sep)
    tra_image_dir = os.path.join('DUTS_train', 'DUTS_imgs' + os.sep)
    tra_label_dir = os.path.join('DUTS_train', 'DUTS_labels' + os.sep)
    ...
    # hyper-parameters
    epoch_num = 30
    batch_size_train = 32  # we have used the 32 as batchsize for training but if cuda runs out of memeory reduce the batchsize
    batch_size_val = 1
    train_num = 0
    val_num = 0
    ...
    if torch.cuda.is_available():
        save_frq = 500  # save the model every 500 iterations if run with gpu
        net.cuda()
    ...
    for epoch in range(0, epoch_num):
        ...
        for i, data in enumerate(salobj_dataloader):
            ...
            wandb.log({"running_tar loss":running_tar_loss / ite_num4val,"running_loss":running_loss / ite_num4val,"epoch":epoch+1})

            if ite_num % save_frq == 0:  
                print("Lets Save the model")
                torch.save(
                    {'epoch': epoch,
                     'ite_num': ite_num,
